...
suite const state_machine_suite = [] {
  ...
  snk::snake_t old_snake{{{3, 3}, {4, 3}}, snk::direction_t::right, board};
  "straight move"_test = [=] {
    ...
    auto state = snk::handle_event(std::move(running_state),
                                   snk::event::timeout{new_time});
  ...
  "collision"_test = [board] {
    auto const cur_time = std::chrono::system_clock::now();
    snk::snake_t given_snake{{{4, 3}, {5, 3}, {6, 3}, {6, 2}, {5, 2}},
                             snk::direction_t::left,
                             board};
    snk::state_t cur_state = snk::running_t{given_snake, {5, 5}, 6, cur_time};
    cur_state = snk::handle_event(std::move(cur_state),
                                  snk::event::direction_change::down);
    auto new_time = cur_time + 1s;
    cur_state =
  ...
  "eating straight"_test = [=] {
    ...
    snk::state_t expected_state = snk::fruit_needed_t{
        snk::running_t{expected_snake, {4, 3}, 6, new_time}};
    expect(eq(std::get<snk::fruit_needed_t>(state).snake(), expected_snake));
    expect(state == expected_state);
    state = snk::handle_event(std::move(state),
                              snk::event::fruit_generated{{7, 3}});
  ...
  "eating with direction"_test = [=] {
    ...
    state = snk::handle_event(std::move(state), snk::event::timeout{new_time});
    snk::snake_t expected_snake{
  ...
  "eating on snake edge"_test = [board] {
    ...
    expect(eq(std::get<snk::fruit_needed_t>(state).snake(), expected_snake));
    snk::state_t expected_state = snk::fruit_needed_t{
        snk::running_t{expected_snake, {6, 2}, 6, new_time}};
    expect(state == expected_state);
    state = snk::handle_event(std::move(state),
                              snk::event::fruit_generated{{9, 9}});
