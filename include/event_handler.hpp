...
struct event_handler {
  ...
  auto operator()(event::start evt, state_t state) {
    ...
    if (!rd::is<snk::closed_t>(state)) {
      ...
      state = snk::running_t{std::move(init_snake), init_fruit_position, speed,
                             evt.cur_time};
    }
    return state;
  }
  ...
  auto operator()(event::timeout timeout, state_t state) -> state_t {
    return std::move(state) |
           ...
           rd::then<running_t>(boost::hof::flow(snk::apply_queued_directions,
                                                snk::try_eating)) |
...
auto handle_event(state_t state, event::event_t event) -> state_t {
  return std::visit<snk::state_t>(
      [app_state = std::move(state)](auto evt) {
        return event_handler{}(evt, std::move(app_state));
      },
      std::move(event));
}
}  // namespace snk
